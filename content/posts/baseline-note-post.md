...
Отсюда проблема:
как в baseline сборке запускать условно "дебажный" Remote Config, чтобы baseline генерировался в нужном окружении и при этом не ломать релиз. На практике сразу вылезло несколько ограничений:
...
SharedPreference тоже. Debug включить тоже нет. Поэтому я проверял поведение в два шага.
